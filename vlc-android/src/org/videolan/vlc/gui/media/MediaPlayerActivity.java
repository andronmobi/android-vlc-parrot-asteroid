...
public class MediaPlayerActivity extends Activity implements IMediaPlayer {
    ...
    public final static String TAG = "VLC/MediaPlayerActivity";
...
//            }}, 1500);
    ...
    protected void onResume() {
        super.onResume();
        Log.d(TAG, "onResume");
//        mMediaController.bindMediaService(this);
//        mMediaController.addMediaPlayer(this);
//        mNotificationManager.cancel(NOTIFY_EXTERNAL_DISPLAY);
    ...
    protected void onPause() {
        ...
        // Get position
        long time = mMediaController.getTime();
        long length = mMediaController.getLength();
...
//            }}, 500);
    ...
    public static void start(Context context, String location, String title, Boolean dontParse, Boolean fromStart) {
        ...
        if (dontParse) {
            ...
            AudioServiceController asc = AudioServiceController.getInstance(); // TODO
            asc.stop();
        }
...
//    }
    ...
    private void fadeOutInfo() {
        if (mInfo.getVisibility() == View.VISIBLE)
            mInfo.startAnimation(AnimationUtils.loadAnimation(
                    MediaPlayerActivity.this, android.R.anim.fade_out));
    ...
    private static class VideoPlayerEventHandler extends WeakHandler<MediaPlayerActivity> {
        public VideoPlayerEventHandler(MediaPlayerActivity owner) {
            super(owner);
        }

        @Override
        public void handleMessage(Message msg) {
            MediaPlayerActivity activity = getOwner();
...
//                    Log.i(TAG, "MediaPlayerPositionChanged pos=" + pos);
                    break;
     ...
     */
    private final Handler mHandler = new MediaPlayerHandler(this);

    // We should use a normal Handler instead of WeakHandler.
    // There is a composition of MediaPlayerActivity and Handler.   
    private static class MediaPlayerHandler extends WeakHandler<MediaPlayerActivity> {
        
        public MediaPlayerHandler(MediaPlayerActivity owner) {
            super(owner);
        }
    ...
    private boolean canShowProgress() {
        return !mDragging && mShowing && mMediaController.isPlaying();
    }
    ...
    private void encounteredError() {
        ...
        AlertDialog dialog = new AlertDialog.Builder(MediaPlayerActivity.this)
    ...
    private void handleVout(Message msg) {
        if (msg.getData().getInt("data") == 1 && !mEndReached) {
            mScreenId = mMediaController.getScreenId();
            if (mScreenId == -1) {
                Log.e(TAG, "Can't detect screen id");
                mScreenId = 0;
            }
        }
//        if (msg.getData().getInt("data") == 0 && !mEndReached) { TODO
...
//        }
    ...
    private final OnSeekBarChangeListener mSeekListener = new OnSeekBarChangeListener() {
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            if (fromUser) {
                mMediaController.setTime(progress);
                setOverlayProgress();
...
//                    if(trackID < 0) return;
//
//                    MediaDatabase.getInstance(MediaPlayerActivity.this).updateMedia(
//                            mLocation,
...
//            dialog.setOwnerActivity(MediaPlayerActivity.this);
//            dialog.show();
//        }
//    };
    ...
    */
    private final OnPlayerControlListener mPlayerControlListener = new OnPlayerControlListener() {
        ...
        public void onSeek(int delta) {
            // unseekable stream
            if(mMediaController.getLength() <= 0) return;

            long position = mMediaController.getTime() + delta;
            if (position < 0) position = 0;
            mMediaController.setTime(position);
            showOverlay();
        }
        ...
        public long onWheelStart() {
            showOverlay(OVERLAY_INFINITE);
            return mMediaController.getTime();
        }
        ...
        public int onScreen() {
            ...
            mMediaController.setScreenId(mScreenId);
            return mScreenId;
        }
    };
    ...
    private final SurfaceHolder.Callback mSurfaceCallback = new Callback() {
        ...
        public void surfaceCreated(SurfaceHolder holder) {
            ...
            Log.d(TAG, "surfaceCreated, mScreenId=" + mScreenId);
            mMediaController.addMediaPlayer(MediaPlayerActivity.this);
            mMediaController.bindMediaService(MediaPlayerActivity.this);
        }
        
        @Override
        public void surfaceDestroyed(SurfaceHolder holder) {
            Log.d(TAG, "surfaceDestroyed");
            if (mScreenId == 0 || MediaPlayerActivity.this.isFinishing()) {
                mMediaController.detachSurface();
            }
            mMediaController.unbindMediaService(MediaPlayerActivity.this);
            mMediaController.removeMediaPlayer(MediaPlayerActivity.this);
            mIsConnected = false;
        }
    };
    ...
    private void updateOverlayPausePlay() {
        ...
        mControls.setState(mMediaController.isPlaying());
    }
    ...
    private int setOverlayProgress() {
        ...
        int time = (int) mMediaController.getTime();
        int length = (int) mMediaController.getLength();
...
//    }
    ...
    private void load() {
        ...
        Log.i(TAG, "load, Location: " + mLocation + ", dontParse=" + dontParse);

        //mSurface.setKeepScreenOn(true);

//        if (!dontParse)
            mMediaController.load(mLocation, 0, false, false);
        ...
        if (mLocation != null && mLocation.length() > 0 && !dontParse) {
            ...
            if(media != null) {
                ...
                if(media.getTime() > 0 && !fromStart)
                    mMediaController.setTime(media.getTime());

//                mLastAudioTrack = media.getAudioTrack(); TODO
//                mLastSpuTrack = media.getSpuTrack();     TODO
            } else {
                ...
                editor.commit();
                if(rTime > 0)
                    mMediaController.setTime(rTime);
            }
    ...
    /**************************************************************************
     * Implementation of IMediaPlayer interface 
     **************************************************************************/
    ...
    /**
     * The method is called when the activity is connected to MediaServer.   
     */
    @Override
    public void onMediaServiceConnect() {
        mIsConnected = true;
        mMediaController.attachSurface(mSurfaceHolder.getSurface(), mVideoWidth, mVideoHeight);
        if (mScreenId == 0) {
            doLoading();
        }
    }
    ...
    /**
     * The method is called when MediaServer is unexpectedly disconnected.   
     */
    @Override
    public void onMediaServiceDisconnect() {
        mIsConnected = false;
        Log.e(TAG, "Connection with MediaServer is lost");
        finish();
    }
    
    /**************************************************************************
     * The end of implementation of IMediaPlayer interface 
     **************************************************************************/
