...
public class MediaService extends Service {
    ...
    private static final String TAG = "VLC/MediaService";
    ...
    private HashMap<IMediaServiceCallback, Integer> mCallback;
    ...
    /**
     * If media is created on the fly.
     */
    private boolean mIsDynamicMedia = false;
    
    /**
     * If video output is existed for the current media.
     */
    private boolean mIsVout = false;
     ...
     */
    private final Handler mVlcEventHandler = new MediaServiceEventHandler(this);

    private static class MediaServiceEventHandler extends WeakHandler<MediaService> {
        public MediaServiceEventHandler(MediaService fragment) {
            super(fragment);
        }

        private boolean mIsVoutStarted = false;

        @Override
        public void handleMessage(Message msg) {
            MediaService service = getOwner();
            ...
            switch (msg.getData().getInt("event")) {
                ...
                case EventHandler.MediaPlayerVout:
                    int voutCount = msg.getData().getInt("data");
                    service.mIsVout = true;
                    Log.i(TAG, "MediaPlayer vout event: " + voutCount);
                    if (voutCount == 1) {
                        service.showNotification();
                    }
                    if(voutCount == 0 && service.mIsDynamicMedia) {
                        if (!mIsVoutStarted && !service.mLibVLCPlaylistActive) {
                            mIsVoutStarted = true;
                            service.handleVout();
                        }
                    } else {
                        mIsVoutStarted = false;
                    }
                    break;
...
//        // Switch to the video player & don't lose the currently playing stream
        MediaPlayerActivity.start(VLCApplication.getAppContext(), mCurrentMedia.getLocation(), mCurrentMedia.getTitle(), true);
    }
    ...
    private final Handler mHandler = new MediaServiceHandler(this);

    private static class MediaServiceHandler extends WeakHandler<MediaService> {
        public MediaServiceHandler(MediaService fragment) {
            super(fragment);
        }
    ...
    private void showNotification() {
        try {
            ...
            Intent notificationIntent = new Intent(this, mIsVout ? MediaPlayerActivity.class : AudioPlayerActivity.class);
    ...
    private final IMediaService.Stub mInterface = new IMediaService.Stub() {
        ...
        public Bitmap getCover() {
            if (mCurrentMedia != null) {
                return MediaService.this.getCover();
            }
            return null;
        }

        @Override
        public synchronized void addMediaCallback(IMediaServiceCallback cb)
        ...
        @Override
        public synchronized void removeMediaCallback(IMediaServiceCallback cb)
                throws RemoteException {
            Integer count = mCallback.get(cb);
            if (count == null)
                count = 0;
        ...
        public void load(List<String> mediaPathList, int position, boolean libvlcBacked, boolean noVideo)
            ...
            if(mLibVLCPlaylistActive) {
                ...
                MediaDatabase db = MediaDatabase.getInstance(MediaService.this);
                for (int i = 0; i < mediaPathList.size(); i++) {
                    String path = mediaPathList.get(i);
                    Media media = db.getMedia(MediaService.this, path);
                    if(media == null) {
                        Log.v(TAG, "Creating on-the-fly Media object for " + path);
                        mIsDynamicMedia = true;
            ...
            if (mCurrentMedia != null) {
                ...
                setUpRemoteControlClient();
                showNotification();
                updateWidget(MediaService.this);
                updateRemoteControlClientMetadata();
            }
            MediaService.this.saveMediaList();
            MediaService.this.saveCurrentMedia();
        }
        ...
        public void setRepeatType(int t) throws RemoteException {
            MediaService.this.setRepeatType(t);
        }
        ...
        private IVideoPlayer player = new IVideoPlayer() {
            @Override
            public void setSurfaceSize(int width, int height, int sar_num, int sar_den) {
                Log.d(TAG, "setSurfaceSize w=" + width + ", h=" + height);
            }
        };
        
        @Override
        public void attachSurface(Surface surface, int width, int height) throws RemoteException {
            Log.d(TAG, "attachSurface");
            mLibVLC.attachSurface(surface, player, width, height);
        }

        @Override
        public void detachSurface() throws RemoteException {
            Log.d(TAG, "detachSurface");
            mLibVLC.detachSurface();
        }

        @Override
        public void setScreenId(int id) throws RemoteException {
            Log.d(TAG, "setScreenId id=" + id);
            mLibVLC.setScreenId(id);
        }

        @Override
        public int getScreenId() throws RemoteException {
            int id = mLibVLC.getScreenId();
            Log.d(TAG, "getScreenId id=" + id);
            return id;
        }
    };
